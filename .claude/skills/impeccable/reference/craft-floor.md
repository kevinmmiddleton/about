...
- **WebKit:** when the build uses `@property` animation, `mask-composite`, `-webkit-mask` glyphs, or `backdrop-filter`, render once in real Safari via the Safari MCP and confirm the effect animates, not just that the property is supported. The viewport width you pass is not the CSS width the page sees (`devicePixelRatio` 2.3 turns 390 into `innerWidth` 339); read `innerWidth` when a breakpoint is close.
