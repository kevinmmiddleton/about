...
- Verify in bounded passes, not a loop: build fully, then inspect with one batched screenshot round (desktop and mobile together), fix everything it shows, and confirm with at most one more round. Per-tweak screenshot cycles burn the user's money without raising the bar; live mode is the channel for tweak-by-tweak iteration. The browser pane is Chromium, so when the work uses `@property` animation, `mask-composite`, `-webkit-mask`, or `backdrop-filter`, fold one real-WebKit round into the same pass via the Safari MCP (`mcp__safari__*`): `navigate_to_url` → `set_viewport_size` → `navigate_to_url` again. The width you pass is not the CSS width the page sees (the tab reports `devicePixelRatio` 2.3 on Kevin's Mac, so 390 lands at `innerWidth` 339 and 1440 at 1252); check `innerWidth` when a breakpoint is close, and pull `browser_console_messages` filtered to warn+error.
